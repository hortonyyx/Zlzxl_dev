...
- 开发 Agent 完成小节点并需要交叉审阅时，必须先在
  `docs/reviews/requests/` 落审阅请求包。
- 审阅意见**必须落成文档**，不能只停留在聊天输出。
- 审阅结果统一放在 `docs/reviews/results/`。
- 请求文件名格式：`YYYY-MM-DD_<目标>_<期望审阅方>_review_request.md`。
- 结果文件名格式：`YYYY-MM-DD_<目标>_<审阅方>_review.md`。
- 审阅请求至少包含：审阅对象、背景、请先读、本轮已改、已验证、请重点审阅、期望输出路径。
- 审阅结果至少包含：
  ...
  - Verdict：是否可进入下一阶段。
  - 处置摘要：已修 / 待办 / 不采纳及理由。
- `docs/reviews/README.md` 的请求 / 结果列表必须同步追加。
- 未归档到 `docs/reviews/results/` 的审阅，不算完成交叉审阅闸门。
